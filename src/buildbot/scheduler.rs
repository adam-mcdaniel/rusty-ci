...
impl Scheduler {
    ...
    fn new<S>(name: S, depends: Option<String>, password: S, branch: S, file_triggers: Vec<S>, buildernames: Vec<S>) -> Self
    where
        S: Display,
    {
        Self {
...
c['schedulers'].append({name})
    ...
    ", name=self.name, depends=depends, buildernames=format!("{:?}", self.buildernames))
            },
            None => {
                write!(
                    f,
